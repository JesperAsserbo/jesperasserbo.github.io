...
//**  CheckMinMaxLimit()
  ...
  //************* NEW ********** 2024.10.17 *********************
  //** Update() CifferPos so that buttonRollor can move *********
  //************* NEW ********** 2024.10.17 *********************
...
class ButtonRollor {
  ...
  Update() {
    ...
    for (let i = 0; i < this.numberSufix; i++) {
      //** Pos Update
      this.buttonCiffer[i].pos.x = this.pos3.x - (1 + i) * this.buttonWidth;
      this.buttonCiffer[i].pos.y = this.pos3.y - this.buttonHeight;

      //** Button Center Update
      this.buttonCiffer[i].buttonCenter.x =
        this.buttonCiffer[i].pos.x + 0.5 * this.buttonWidth;

      this.buttonCiffer[i].buttonCenter.y =
        this.buttonCiffer[i].pos.y + 5 + 0.5 * this.buttonHeight - 2.5;
    }

    for (let i = this.numberSufix; i < this.buttonCiffer.length; i++) {
      //**If no sufix then correct for ","
      let correction = 10;
      if (this.numberSufix == 0) correction = 0;

      //** Pos Update
      this.buttonCiffer[i].pos.x =
        this.pos3.x - correction - (1 + i) * this.buttonWidth;
      this.buttonCiffer[i].pos.y = this.pos3.y - this.buttonHeight;
    ...
    //** Text
    this.pos1.y = this.pos3.y;
    this.pos2.y = this.pos3.y;
    this.pos4.y = this.pos3.y;
  }
